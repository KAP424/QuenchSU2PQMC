...
function ctrl_SCEEicr(path::String,model::_Hubbard_Para,indexA::Vector{Int64},indexB::Vector{Int64},Sweeps::Int64,λ::Float64,Nλ::Int64,ss::Vector{Matrix{UInt8}},record)
    ...
    file="$(path)Quench_SCEE$(name)_t$(model.t)R$(model.Rate)size$(model.site)Δt$(model.Δt)N$(Nλ)BS$(model.BatchSize).csv"
    ...
    atexit() do
        ...
        writedlm("$(path)ss/SS$(name)_t$(model.t)R$(model.Rate)size$(model.site)Δt$(model.Δt)λ$(Int(round(Nλ*λ))).csv", [ss[1] ss[2]],",")
    end
    ...
    for loop in 1:Sweeps
        ...
        for lt in model.Nt-1:-1:1
            if mod(lt,model.WrapTime)==0 || lt==div(model.Nt,2)
                ...
                Gt1=model.eKinv*diagm(exp.(-1im*model.α[lt+1].*D1)) *Gt1* diagm(exp.(1im*model.α[lt+1].*D1))*model.eK 
                Gt2=model.eKinv*diagm(exp.(-1im*model.α[lt+1].*D2)) *Gt2* diagm(exp.(1im*model.α[lt+1].*D2))*model.eK
                G0t1=G0t1*diagm(exp.(1im*model.α[lt+1].*D1))*model.eK
                G0t2=G0t2*diagm(exp.(1im*model.α[lt+1].*D2))*model.eK
                Gt01=model.eKinv*diagm(exp.(-1im*model.α[lt+1].*D1))*Gt01
                Gt02=model.eKinv*diagm(exp.(-1im*model.α[lt+1].*D2))*Gt02
