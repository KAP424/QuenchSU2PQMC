...
function ctrl_EEicr(path::String,model::_Hubbard_Para,index::Vector{Int64},Sweeps::Int64,λ::Float64,Nλ::Int64,ss::Vector{Matrix{UInt8}},record)::Vector{Matrix{UInt8}}
    ...
    file = "$(path)QuenchEEicr$(name)_t$(model.t)V$(model.V_quench)size$(model.site)Δt$(model.Δt)Θ$(model.Θ)N$(Nλ)BS$(model.BatchSize).csv"
    atexit() do
        if record
            open(file, "a") do io
                lock(io)
        ...
        writedlm("$(path)ss/QuenchEEicr$(name)_t$(model.t)V$(model.V_quench)size$(model.site)Δt$(model.Δt)Θ$(model.Θ)λ$(Int(round(Nλ*λ))).csv", [ss[1] ss[2]],",")
    end
    ...
    for loop in 1:Sweeps
        for lt in 1:model.Nt
            if mod(lt,model.WrapTime)==1 || lt==div(model.Nt,2)+1
                ...
                Gt1=diagm(exp.(1im*model.α[lt].*D1))*model.eK *Gt1* model.eKinv*diagm(exp.(-1im*model.α[lt].*D1))
                Gt2=diagm(exp.(1im*model.α[lt].*D2))*model.eK *Gt2* model.eKinv*diagm(exp.(-1im*model.α[lt].*D2))
                G0t1=G0t1*model.eKinv*diagm(exp.(-1im*model.α[lt].*D1))
                G0t2=G0t2*model.eKinv*diagm(exp.(-1im*model.α[lt].*D2))
            ...
            for x in 1:model.Ns
                ...
                Δ1=exp(1im*model.α[lt]*(model.η[sx1]-model.η[ss[1][x,lt]]))-1
                r1=1+Δ1*(1-Gt1[x,x])
                ...
                Δ2=(exp(1im*model.α[lt]*(model.η[sx2]-model.η[ss[2][x,lt]]))-1)
                r2=(1+Δ2*(1-Gt2[x,x]))
            ...
            ##------------------------------------------------------------------------
            O[Int(NO+0.5-abs(lt-NO-0.5)),loop+2]+=detg
            counter[Int(NO+0.5-abs(lt-NO-0.5))]+=1
            ##------------------------------------------------------------------------
        end
        ...
        for lt in model.Nt-1:-1:1
            if mod(lt,model.WrapTime)==0 || lt==div(model.Nt,2)
                ...
                Gt1=model.eKinv*diagm(exp.(-1im*model.α[lt].*D1)) *Gt1* diagm(exp.(1im*model.α[lt].*D1))*model.eK 
                Gt2=model.eKinv*diagm(exp.(-1im*model.α[lt].*D2)) *Gt2* diagm(exp.(1im*model.α[lt].*D2))*model.eK
                G0t1=G0t1*diagm(exp.(1im*model.α[lt].*D1))*model.eK
                G0t2=G0t2*diagm(exp.(1im*model.α[lt].*D2))*model.eK
                Gt01=model.eKinv*diagm(exp.(-1im*model.α[lt].*D1))*Gt01
                Gt02=model.eKinv*diagm(exp.(-1im*model.α[lt].*D2))*Gt02
...
#                 D1=[model.η[x] for x in ss[1][:,lt]]
#                 D2=[model.η[x] for x in ss[2][:,lt]]
#                 G1=diagm(exp.(1im*model.α[lt].*D1))*model.eK *G1* model.eKinv*diagm(exp.(-1im*model.α[lt].*D1))
#                 G2=diagm(exp.(1im*model.α[lt].*D2))*model.eK *G2* model.eKinv*diagm(exp.(-1im*model.α[lt].*D2))
...
#                 sp=Random.Sampler(rng,[i for i in elements if i != ss[1][x,lt]])
#                 sx=rand(rng,sp)
#                 Δ=exp(1im*model.α[lt]*(model.η[sx]-model.η[ss[1][x,lt]]))-1
#                 r=1+Δ*(1-G1[x,x])
...
#                 sp=Random.Sampler(rng,[i for i in elements if i != ss[2][x,lt]])
#                 sx=rand(rng,sp)
#                 Δ=exp(1im*model.α[lt]*(model.η[sx]-model.η[ss[2][x,lt]]))-1
#                 r=1+Δ*(1-G2[x,x])
...
#                         G01= model.eKinv*diagm(exp.(-1im*model.α[lt].*D1)) *G01*  diagm(exp.(1im*model.α[lt].*D1))*model.eK
#                         G02= model.eKinv*diagm(exp.(-1im*model.α[lt].*D2)) *G02*  diagm(exp.(1im*model.α[lt].*D2))*model.eK
#                     end
#                 else
#                     for i in lt+1:div(model.Nt,2)
#                         D1=[model.η[x] for x in ss[1][:,i]]
#                         D2=[model.η[x] for x in ss[2][:,i]]
#                         G01=diagm(exp.(1im*model.α[lt].*D1))*model.eK *G01* model.eKinv*diagm(exp.(-1im*model.α[lt].*D1))  
#                         G02=diagm(exp.(1im*model.α[lt].*D2))*model.eK *G02* model.eKinv*diagm(exp.(-1im*model.α[lt].*D2))  
#                     end
#                 end
#                 #####################################################################
...
#                 D1=[model.η[x] for x in ss[1][:,lt+1]]
#                 D2=[model.η[x] for x in ss[2][:,lt+1]]
#                 G1=model.eKinv*diagm(exp.(-1im*model.α[lt].*D1)) *G1* diagm(exp.(1im*model.α[lt].*D1))*model.eK 
#                 G2=model.eKinv*diagm(exp.(-1im*model.α[lt].*D2)) *G2* diagm(exp.(1im*model.α[lt].*D2))*model.eK 
